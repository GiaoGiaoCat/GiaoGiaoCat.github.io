...
* __输入 /dn 显示 symbols.yaml 中定义的符号；__
* __识别网址、邮件地址，输入后直接上屏；__
* __常见拼写容错处理；__
* __支持词义关联模式的表情符号、颜文字；__
* __#、@、`、等 MarkDown 符号直接上屏，单引号输出「」；__
* __- +键 < > 翻页，上下箭头选择其他候选；__
* __用 ← 键移动光标到第一个字拼音的结束位置，这时的候选字列表只包含第一个字的同音字。__
* __可以用（Shift+）← 按音节向左移动光标，还可以 Shift+→ 从输入串末尾末尾跳回第一个音节。__
* __Tab、Shift+Tab 映射为 Shift+左右方向键__
